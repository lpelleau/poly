...
inline HuffOutput<T> mat2huff(std::vector<T> vec) {
    ...
    for(size_t i=0; i<vec.size(); ++i) {
        //Does not work with c++ 14 or c++ 11
        //auto pos = (std::make_unsigned<T>::type) vec[i];
        //Manually shift -> 32768 = MAX_INT_16 + 1
        uint16_t pos = vec[i] + 32768;
        ++anSymbolFrequencies[pos];
    }
